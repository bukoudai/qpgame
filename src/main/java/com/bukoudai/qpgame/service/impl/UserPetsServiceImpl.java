...
public class UserPetsServiceImpl extends ServiceImpl<UserPetsMapper, UserPets> implements UserPetsService, IService<UserPets> {
  ...
  public List<UserPets> queryUserPets(User user) {
    ...
    return baseMapper.queryUserPets(user);
  }
  ...
  public List<String> pk(UserPets one, String nick, UserPets other) {
    ...
    while (one1.getHp() > 0 && other1.getHp() > 0) {
      ...
      if (((ackoneSeep) - ackBaseSeep >= 0) && ack(nick, "对方", one1, other1, re)) {
        break;
      }
      //2到达攻击时机
      ackothSeep = ackothSeep + speed2;
      if (((ackothSeep) - ackBaseSeep >= 0) && ack("对方", nick, other1, one1, re)) {
        break;
      }
    }
  ...
  private boolean ack(String nick, String nick2, Pets one1, Pets other1, List<String> re) {
    ...
    if (hpd > 0) {
      ...
      other1.setHp(i);
      String s = format("{}的宠物{} 进行攻击,{}的宠物{}受到{}点伤害 现有hp:{}", nick, one1.getPetName(), nick2, other1.getPetName(), hpd, i);
      ...
      other1.setHp(i);
      String s = format("{}的宠物{} 进行攻击,未能击破护甲,{}的宠物{}受到1点伤害 现有hp:{}", nick, one1.getPetName(), nick2, other1.getPetName(), i);
      re.add(s);
      if (i <= 0) {
        re.add(nick2 + "的宠物已战败");
        return true;
